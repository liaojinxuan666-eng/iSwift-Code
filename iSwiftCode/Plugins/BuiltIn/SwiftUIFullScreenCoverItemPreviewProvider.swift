...
/// Adds constrained source support for `.fullScreenCover(item:)` above the existing
/// item-driven Sheet and presentation provider stack.
///
/// Optional primitive state remains portable Preview IR. The signed runtime
/// bridges that state to SwiftUI's native item-driven full-screen presentation.
final class SwiftUIFullScreenCoverItemPreviewProvider: PreviewProvider {
    private let base = SwiftUISheetItemPreviewProvider()
    ...
    private func makePreview(
        ...
        guard presentationDepth <= 12 else {
            return diagnosticResult(
                PreviewFullScreenCoverItemError.maximumDepthExceeded,
        ...
        let rewrite: PreviewFullScreenCoverItemRewrite

        do {
            rewrite = try PreviewFullScreenCoverItemSourceRewriter(
    ...
    private func parseCovers(
        _ specs: [PreviewFullScreenCoverItemSpec],
        statePrelude: String,
        definitions: [PreviewStateDefinition],
        originalPath: String,
        request: PreviewRequest,
        presentationDepth: Int
    ) throws -> [String: PreviewFullScreenCoverItemResolved] {
        var result: [String: PreviewFullScreenCoverItemResolved] = [:]

        for spec in specs {
            guard let definition = definitions.first(
                where: { $0.name == spec.stateName }
            ) else {
                throw PreviewFullScreenCoverItemError.unknownState(
                    spec.stateName
                )
            }
            ...
            guard isOptionalPrimitive(
                ...
                throw PreviewFullScreenCoverItemError.requiresOptionalPrimitive(
                    spec.stateName
                )
            }
            ...
            guard let contentDocument = contentResult.document,
                ...
                let message = contentResult.diagnostics
                    ...
                    .message ??
                    "Full-screen cover item content could not be previewed."

                throw PreviewFullScreenCoverItemError.invalidContent(
                    stateName: spec.stateName,
                    message: message
                )
            }

            result[spec.marker] = PreviewFullScreenCoverItemResolved(
                stateName: spec.stateName,
    ...
    private func replacingSheetMarkers(
        in node: PreviewNode,
        sheets: [String: PreviewFullScreenCoverItemResolved]
    ) -> PreviewNode {
        switch node {
...
private struct PreviewFullScreenCoverItemResolved {
    let stateName: String
    let content: PreviewNode
}

private struct PreviewFullScreenCoverItemSpec {
    let marker: String
    let stateName: String
    let itemName: String
...
private struct PreviewFullScreenCoverItemRewrite {
    let source: String
    let sheets: [PreviewFullScreenCoverItemSpec]
    let statePrelude: String
}

private enum PreviewFullScreenCoverItemError: Error {
...
extension PreviewFullScreenCoverItemError: LocalizedError {
    var errorDescription: String? {
        switch self {
        case .malformedHeader:
            return "Malformed full-screen cover item preview. Use .fullScreenCover(item: $state) { item in ... }."

        case .malformedContent(let stateName):
            return "Full-screen cover item bound to '\(stateName)' must use a single closure parameter, for example { item in ... }."

        case .unknownState(let stateName):
            return "Full-screen cover item references unknown @State '\(stateName)'."

        case .requiresOptionalPrimitive(let stateName):
            return "Full-screen cover item binding '\(stateName)' must reference typed optional String, Bool, Int, Double, or Float @State initialized to nil."

        case .invalidContent(
            let stateName,
            let message
        ):
            return "Full-screen cover item '\(stateName)' content error: \(message)"

        case .maximumDepthExceeded:
            return "Full-screen cover item preview exceeded the maximum supported presentation depth."
        }
    }
}

private struct PreviewFullScreenCoverItemSourceRewriter {
    let source: String

    func rewrite() throws -> PreviewFullScreenCoverItemRewrite {
        var output = source
        var sheets: [PreviewFullScreenCoverItemSpec] = []

        while true {
            let scanner = PreviewFullScreenCoverItemScanner(
                source: output
            )
        ...
        return PreviewFullScreenCoverItemRewrite(
            source: output,
            sheets: sheets,
...
private struct PreviewFullScreenCoverItemScanner {
    let source: String
    ...
    func rewriteFirst(
        ...
        source: String,
        spec: PreviewFullScreenCoverItemSpec
    )? {
        ...
        while let start = nextCoverStart(
            ...
            if let parsed {
                let marker =
                    "__ISWIFT_FULL_SCREEN_ITEM_\(markerIndex)__"
                ...
                return (
                    rewritten,
                    PreviewFullScreenCoverItemSpec(
                        marker: marker,
    ...
    private func parseCover(
        ...
        guard cursor < source.endIndex,
              source[cursor] == "{" else {
            throw PreviewFullScreenCoverItemError
    ...
    private func parseStateName(
        ...
        let range = Range(
            ...
            throw PreviewFullScreenCoverItemError.malformedHeader
        }
